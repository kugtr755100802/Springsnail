...
int main( int argc, char* argv[] )
{
	...
	vector< host > logical_srv;     //将xml文件中的host解析到vector中
    host tmp_host;
	...
	/*
        ...
        这段代码使用了有限状态机的思想，当我们遇到< logical_host >的时候我们将optag设置为true,代表开始解析;
        然后通过\n来对buf中所保存的信息进行处理，从而读入所要代理的服务器的ip地址端口号以及所要建立的连接数。
        遇到< /logical_host >时，代表一个服务器的信息已经读完，然后将optag设置为false。如此往复直到读到buf的结尾。
        （Listen的读取大家可以自行查看，是有错误的，我在这里将其改正了一下）。
	...
	*/
    while( tmp2 = strpbrk( tmp, "\n" ) )  //在源字符串tmp中找出最先含有搜索字符串"\n"中任一字符的位置并返回，若没找到则返回空指针
    {
        *tmp2++ = '\0';
        ...
        else if( tmp3 = strstr( tmp, "<name>" ) )   //将tmp_hostname指针指向<name>后面的IP地址的首个地址
        {
            tmp_hostname = tmp3 + 6;
